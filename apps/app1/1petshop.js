...
App.controller('ctrl', function($scope, $http) {
    ...
    $scope.savePet = function(pet) {
        $http.put('http://localhost:8080/pet/' + pet.id + '?name=' + pet.name + '&type=' + pet.type).then(function(resp) {
            var response = resp.data;
            if (response && response.success) {
                alert('ok');
            }
...
App.factory('PetService', function($http) {
    var addPet = function(name, type) {
        return $http.post('http://localhost:8080/pet?name=' + name + '&type=' + type.toUpperCase());
    };
});